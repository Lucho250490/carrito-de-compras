import { productos, regalos, pintarProductos } from "./products.js"
...
const regaloList = document.querySelector('.gift__list')
...
function pintarRegalo() {
  ...
  for (let articuloR of regalo) {
    const productoR = regalos.find(r => r.id === articuloR.id)

    html2 += `<article class="cart__item">
    <div class="cart__img">
    ...
    <div class="cart__content">
      <h2 class="cart__name">
        ...
        <span class="cart__price">$${productoR.precio}</span>
      </h2>
      <div class="cart__amout">
        <div>
          <span class="cart__subtotal">$${productoR.precio * articuloR.cantidad}</span> x <span class="cart__quantity">${articuloR.cantidad}</span>
        </div>
        <button type="button" class="button cart__button cart--delete" title="remove from cart" data-id="${articuloR.id}">
          <i class='bx bx-trash button--icon'></i>
        </button>
      </div>
    </div>
    <div class="cart__buttons">
      <button type="button" class="button cart__button cart--remove" title="remove one" data-id="${articuloR.id}">
        <i class='bx bx-minus button--icon'></i>
      </button>
      <span class="cart__quantity">${articuloR.cantidad}</span>
      <button type="button" class="button cart__button cart--add" title="add one" data-id="${articuloR.id}">
        <i class='bx bx-plus button--icon'></i>
      </button>
    </div>
  </article>`
